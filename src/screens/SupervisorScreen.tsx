...
export default function SupervisorScreen({ navigation }: any) {
  ...
  const handleClearJornada = async () => {
    Alert.alert(
      '⚠️ Limpieza de Jornada (Solo Transacciones)',
      '¿Confirmas que deseas vaciar TODOS los conteos y asignaciones de hoy? Esto reiniciará la jornada operativa sin borrar a los usuarios.',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'SÍ, LIMPIAR JORNADA',
          style: 'destructive',
          onPress: async () => {
            setLoading(true);
            try {
              ...
              Alert.alert('Jornada Limpia', 'Se han vaciado todos los conteos y asignaciones operativas.');
              fetchActiveAssignments();
            } catch (err: any) {
              Alert.alert('Error al limpiar', err.message);
            } finally {
              setLoading(false);
            }
          }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.userInfoRow}>
          <View style={styles.avatar}>
            <User color="#121212" size={24} />
          </View>
          <View style={styles.userDetails}>
            <Text style={styles.userName}>{user?.nombre}</Text>
            <Text style={styles.userRole}>SUPERVISOR • #{user?.codigo_empleado}</Text>
          </View>
      ...
      {activeTab === 'operaciones' && (
        <ScrollView contentContainerStyle={styles.tabContent} bounces={false}>
          ...
          <TouchableOpacity 
            ...
            onPress={handleClearJornada}
            disabled={loading}
          >
            {loading ? <ActivityIndicator color="#ff4444" size="small" /> : <Trash2 color="#ff4444" size={28} />}
            <View style={styles.actionCardTextContainer}>
              <Text style={styles.dangerCardTitle}>Limpiar Datos de Jornada</Text>
              <Text style={styles.dangerCardSub}>Borra conteos y asignaciones (mantiene usuarios)</Text>
            </View>
          </TouchableOpacity>
        </ScrollView>
      )}
          ...
          >
            <View style={styles.modalOverlay}>
              <View style={styles.modalCard}>
                <Text style={styles.modalTitle}>SELECCIONAR AUXILIAR</Text>
                <FlatList
                  data={usuarios.filter(u => u.rol === 'auxiliar' && u.estado === 'activo')}
                  keyExtractor={(item) => item.id}
                  style={{ maxHeight: 300, marginBottom: 15 }}
                  ListEmptyComponent={
                    <Text style={[styles.grayText, { textAlign: 'center', marginVertical: 20 }]}>
                      No hay operarios auxiliares activos.
                    </Text>
                  }
                  renderItem={({ item }) => (
                    <TouchableOpacity
      ...
      {/* Modal Agregar Usuario (Solo Auxiliar) */}
      <Modal
            ...
            <View style={styles.modalCard}>
              ...
              <View style={styles.switchRow}>
                ...
                <Switch
                  ...
                  disabled={loading}
                  trackColor={{ false: '#333', true: '#e6a822' }}
...
const styles = StyleSheet.create({
  ...
  pendingBadge: {
    backgroundColor: 'rgba(230, 168, 34, 0.08)',
    borderWidth: 1,
    borderColor: 'rgba(230, 168, 34, 0.3)',
    borderRadius: 6,
    paddingHorizontal: 8,
    paddingVertical: 4,
  },
